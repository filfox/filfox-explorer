<template>
  <div class="bg-socialBg social">
    <div class="relative flex items-end h-56">
      <img src="~/assets/img/social/power-growth-header.png" class="absolute w-full top-0">
      <span class="text-white text-sm mx-auto text-center mb-2">
        Filfox.info 更新于 {{ topMinersByPowerDelta.timestamp | timestamp('datetime') }} ({{ currentNetwork.name }})
      </span>
    </div>
...
export default {
  ...
  mounted() {
    QrCodeWithLogo.toCanvas({
      ...
      content: `${this.currentNetwork.url}/ranks/power-growth?utm_source=picshare&utm_campaign=ranks`,
      width: 110,
      logo: {
