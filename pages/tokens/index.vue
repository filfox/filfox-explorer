  ...
  <div class="container mx-auto">
    ...
    <div class="my-4 lg:rounded-md bg-white text-xs md:text-sm">
      <div class="flex items-center px-4 lg:px-10 h-14 text-customGray-450 text-xs">
        <IconPie class="mr-1.5" />{{ $t('home.tokenList.totalTokens', { count: totalCount }) }}
      </div>
      ...
      <ul v-loading="loading" class="lg:hidden px-4">
        ...
        >
          <div class="flex justify-between">
            <span class="text-gray-600">{{ $t('home.tokenList.tableColumns.0') }}</span>
            <span class="flex items-center" @click="$router.push(localePath(`/token/${token.address}`))">
              <img :src="token.tokenIcon" class="w-4 h-4 rounded-full">
          ...
          <div class="flex justify-between">
            ...
            <span>{{ token.totalSupply | parseToken(token.decimals) }}</span>
          </div>
          <div class="flex justify-between">
            <span class="text-gray-600">{{ $t('home.tokenList.tableColumns.2') }}</span>
            <span>{{ token.transferCount }}</span>
          </div>
          <div class="flex justify-between">
            <span class="text-gray-600">{{ $t('home.tokenList.tableColumns.3') }}</span>
...
export default {
  ...
  methods: {
    async getList() {
      this.loading = true
      const { tokens, totalCount } = await this.$axios.$get('/token/erc20-list', {
        params: { offset: this.pageSize * (this.page - 1), limit: this.pageSize }
      })
      this.loading = false
      this.list = tokens.map(token => ({
        ...token,
        tokenIcon: TOKEN_ICONS[token.address] || TOKEN_ICONS.DEFAULT
      }))
  ...
  head() {
    return {
      title: this.$t('meta.titles.tokens')
    }
  }
}
</script>
