...
import { ADMIN_SERVER } from '@/filecoin/filecoin.config'

export default {
  async asyncData({ $axios }) {
    const { result } = await $axios.$get(`${ADMIN_SERVER}/api/blog/category/list`)
  ...
  methods: {
    async getNotifyList() {
      this.loading = true
      const data = await this.$axios.$get(`${ADMIN_SERVER}/api/blog/list`, { params: this.params })
      this.loading = false
    ...
    async getAllIds() {
      const { result: { blogs } } = await this.$axios.$get(`${ADMIN_SERVER}/api/blog/list`, { params: { limit: 100000 } })
