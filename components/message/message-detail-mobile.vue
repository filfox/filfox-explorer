<template>
  <div class="flex flex-grow flex-col bg-white pb-2">
      ...
      <div class="flex flex-row justify-between items-center text-xs mx-4 mt-2">
          <p class="flex w-1/4">{{ $t('detail.message.headers.height') }}</p>
          <TipsetLink :id="message.height" class=" text-main flex w-3/4" />
      </div>

      <div class="flex flex-row justify-between items-center text-xs mx-4 mt-2">
          <p class="flex w-1/4">{{ $t('detail.message.headers.time') }}</p>
          <p class="flex w-3/4"> {{ message.timestamp | timestamp }} </p>
      </div>
      ...
      <div class="flex flex-row justify-between items-center text-xs mx-4 pt-2">
          <p class="flex w-1/4">{{ $t('detail.message.headers.version') }}</p>
          <p class="flex w-3/4"> {{ message.version }} </p>
      </div>

      <div class="flex flex-row justify-between items-center text-xs mx-4 pt-2">
          <p class="flex w-1/4">{{ $t('detail.message.headers.from') }} </p>
          <AddressLink :id="message.from" class="text-main w-3/4"/>
      </div>

      <div class="flex flex-row justify-between items-center text-xs mx-4 pt-2">
          <p class="flex w-1/4">{{ $t('detail.message.headers.to') }}</p>
          <AddressLink :id="message.to" class="text-main w-3/4"/>
      </div>

      <div class="flex flex-row justify-between items-center text-xs mx-4 pt-2">
          <p class="flex w-1/4">{{ $t('detail.message.headers.nonce') }}</p>
          <p class="flex w-3/4"> {{ message.nonce }} </p>
      </div>

      <div class="flex flex-row justify-between items-center text-xs mx-4 pt-2">
          <p class="flex w-1/4">{{ $t('detail.message.headers.method') }}</p>
          <p class="flex w-3/4"> {{ message.method }} </p>
      </div>
      ...
      <div class="flex flex-row justify-between items-center text-xs mx-4 pt-2">
          <p class="flex w-1/4">{{ $t('detail.message.headers.gasPrice') }}</p>
          <p class="flex w-3/4"> {{ message.gasPrice | filecoin() }} </p>
      </div>

      <div class="flex flex-row justify-between items-center text-xs mx-4 pt-2">
          <p class="flex w-1/4">{{ $t('detail.message.headers.gasLimit') }}</p>
          <p class="flex w-3/4"> {{ message.gasLimit | locale }} </p>
      </div>
