<template>
  <div class="lg:rounded-md bg-white">
    <div class="flex flex-row justify-between items-center border-b border-background">
      <div class="h-12 flex items-center pl-4 lg:pl-6 text-xs lg:text-base">
...
export default {
  data() {
    this.chartSettings = {
      ...
      legendName: {},
      labelMap: {
        fil: this.$t('chart.miner.headers.sectorInitialPledgeVariations')
      }
    }
    this.chartExtend = {
      yAxis: {
      ...
      tooltip: {
        ...
        formatter: params => [
          ...
          ...params.map(param => `${param.marker}${param.seriesName}: ${param.value[1]} FIL/32GiB`)
        ].join('<br/>')
      }
    }
    return {
  ...
  methods: {
    async getLineChartData() {
      this.loading = true
      this.rawData = await this.$axios.$get('stats/miner/sector-initial-pledge', { params: { duration: this.duration } })
